...
void tt_log_journal_write(struct tt_log_state* state, enum tt_log_level level, const char* file,
  ...
  if (file && func) {
    sd_journal_send("MESSAGE=%s", buf, "PRIORITY=%d", (int)level, "SYSLOG_IDENTIFIER=%s",
                    state->ident, "CODE_FILE=%s", file, "CODE_LINE=%d", line, "CODE_FUNC=%s", func,
                    NULL);
  } else {
    sd_journal_send("MESSAGE=%s", buf, "PRIORITY=%d", (int)level, "SYSLOG_IDENTIFIER=%s",
                    state->ident, NULL);
...
void tt_log_journal_shutdown(struct tt_log_state* state) { /* No cleanup needed for journal */ }
