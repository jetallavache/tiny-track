...
int tt_log_stderr_init(struct tt_log_state* state) {
  state->backend_data = (state->backend == TT_LOG_BACKEND_STDERR) ? (void*)stderr : (void*)stdout;
  return 0;
}

void tt_log_stderr_write(struct tt_log_state* state, enum tt_log_level level, const char* file,
  ...
  if (state->backend == TT_LOG_BACKEND_DOCKER) {
    ...
    struct tm tm;
    localtime_r(&now, &tm);
    fprintf(out, "[%04d-%02d-%02d %02d:%02d:%02d] [%s] [%s] ", tm.tm_year + 1900, tm.tm_mon + 1,
            tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, state->ident, tt_log_level_str(level));
  }

  if (file && func) fprintf(out, "[%s:%d:%s] ", file, line, func);
