...
static void build_paths(void) {
  ...
  snprintf(p_hostname, sizeof(p_hostname), "%s/sys/kernel/hostname", g_proc_root);
  snprintf(p_ostype, sizeof(p_ostype), "%s/sys/kernel/ostype", g_proc_root);
  snprintf(p_osrelease, sizeof(p_osrelease), "%s/sys/kernel/osrelease", g_proc_root);
}

void tt_sysfs_init(void) {
  const char* env;
  if ((env = getenv("TT_PROC_ROOT")) != NULL) strncpy(g_proc_root, env, BASE_MAX_LEN - 1);
  if ((env = getenv("TT_ROOTFS_PATH")) != NULL) strncpy(g_rootfs_path, env, BASE_MAX_LEN - 1);
  build_paths();
}
...
void tt_sysfs_set_rootfs_path(const char* path) { strncpy(g_rootfs_path, path, BASE_MAX_LEN - 1); }
...
const char* tt_sysfs_stat(void) { return p_stat; }
const char* tt_sysfs_meminfo(void) { return p_meminfo; }
const char* tt_sysfs_net_dev(void) { return p_net_dev; }
const char* tt_sysfs_loadavg(void) { return p_loadavg; }
const char* tt_sysfs_uptime(void) { return p_uptime; }
const char* tt_sysfs_hostname(void) { return p_hostname; }
const char* tt_sysfs_ostype(void) { return p_ostype; }
const char* tt_sysfs_osrelease(void) { return p_osrelease; }
