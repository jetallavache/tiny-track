...
void tt_timer_init(struct tt_timer** head, struct tt_timer* t, uint64_t ms, unsigned flags,
...
int tt_timerfd_create(uint32_t interval_ms) {
  ...
  struct itimerspec its = {
      .it_interval = {.tv_sec = interval_ms / 1000, .tv_nsec = (interval_ms % 1000) * 1000000},
