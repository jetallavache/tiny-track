...
static void test_ini_read(void) {
  ...
  if (!path) {
    printf("  [SKIP] mkstemps failed\n");
    return;
  }

  char buf[64];
  CHECK("read str_key", tt_config_ini_read(path, "section.str_key", buf, sizeof(buf)) == 0 &&
                            strcmp(buf, "hello world") == 0);

  CHECK(
      "read int_key as string",
      tt_config_ini_read(path, "section.int_key", buf, sizeof(buf)) == 0 && strcmp(buf, "42") == 0);
...
static void test_config_read(void) {
  ...
  CHECK("read_int int_key = 42", tt_config_read_int(path, "section.int_key", 0) == 42);

  CHECK("read_int missing → default", tt_config_read_int(path, "section.missing", 99) == 99);

  CHECK("read_bool true", tt_config_read_bool(path, "section.bool_true", false) == true);

  CHECK("read_bool false", tt_config_read_bool(path, "section.bool_false", true) == false);
