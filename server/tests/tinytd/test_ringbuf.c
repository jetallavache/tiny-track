...
static void cleanup(void) { unlink(LIVE_PATH); unlink(SHADOW_PATH); }
...
static void test_write_read(void) {
    ...
    CHECK("get_latest OK",
          ttr_reader_get_latest(&r, &out, sizeof(out)) == TTR_READER_OK);
...
static void test_aggregate_l2(void) {
    ...
    CHECK("aggregate_l2 returns OK or NODATA",
          rc == TTR_WRITER_OK || rc == TTR_WRITER_ERR_NODATA);
...
static void test_null_args(void) {
    printf("\n[null / invalid args]\n");
    CHECK("write_l1 NULL ctx returns error",
          ttr_writer_write_l1(NULL, NULL) != TTR_WRITER_OK);

    cleanup();
    struct ttr_writer w;
    struct ttr_writer_config cfg = make_cfg();
    ttr_writer_init(&w, &cfg);
    CHECK("write_l1 NULL sample returns error",
          ttr_writer_write_l1(&w, NULL) != TTR_WRITER_OK);
    ttr_writer_cleanup(&w);
    cleanup();
}
