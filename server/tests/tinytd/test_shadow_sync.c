...
static int files_identical(const char *a, const char *b) {
    ...
    if (fda < 0 || fdb < 0) { close(fda); close(fdb); return 0; }
    ...
    int eq = (ma != MAP_FAILED && mb != MAP_FAILED)
             ? memcmp(ma, mb, (size_t)sa.st_size) == 0 : 0;
...
#define N_SYNCS 50
    ...
    CHECK("vctx switches within limit",
          dvctx <= (long)(MAX_VCTX_PER_SYNC * N_SYNCS));
    CHECK("no major page faults",
          dmajflt <= (long)(MAX_MAJFLT_PER_SYNC * N_SYNCS));
    CHECK("shadow file identical to live",
          files_identical(LIVE_PATH, SHADOW_PATH));
