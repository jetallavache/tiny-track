...
static void test_fmt_ts(void) {
  ...
  CHECK("fmt_ts non-zero → contains colons", buf[2] == ':' && buf[5] == ':');
...
static void test_color(void) {
  ...
  ctx.color = false;
  CHECK("color disabled → empty string", strcmp(ttc_color(&ctx, COL_GREEN), "") == 0);

  ctx.color = true;
  CHECK("color enabled → non-empty", strlen(ttc_color(&ctx, COL_GREEN)) > 0);
}
...
static void test_pct_color(void) {
  ...
  CHECK("pct_color 30% (warn=70) → green", strstr(c, "32") != NULL); /* \033[32m */

  /* between warn and crit → yellow */
  c = ttc_pct_color(&ctx, 80.0, 70.0, 90.0);
  CHECK("pct_color 80% (warn=70,crit=90) → yellow", strstr(c, "33") != NULL); /* \033[33m */

  /* above crit → red */
  c = ttc_pct_color(&ctx, 95.0, 70.0, 90.0);
  CHECK("pct_color 95% (crit=90) → red", strstr(c, "31") != NULL); /* \033[31m */
}
