...
static void bench_reader_throughput(void) {
  ...
  uint64_t t0 = get_ns();
  for (int i = 0; i < BENCH_ITERATIONS; i++) ttr_reader_get_latest(&r, &out, sizeof(out));
  uint64_t t1 = get_ns();
...
static void* reader_fn(void* arg) {
  ...
  ttr_reader_open(&r, LIVE_PATH);
  uint64_t t0 = get_ns();
  for (int i = 0; i < ctx->iters; i++) ttr_reader_get_latest(&r, &out, sizeof(out));
...
static void bench_concurrent_readers(void) {
  ...
  for (int i = 0; i < NUM_READERS; i++)
    printf("  Thread %d: %.0f ops/sec\n", i, ctxs[i].iters / (ctxs[i].dur_ns / 1e9));
  printf("\n");
}
