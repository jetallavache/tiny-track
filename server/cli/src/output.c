...
void ttc_print_metrics(const struct ttc_ctx* ctx, const struct tt_metrics* m) {
  ...
  if (ctx->format == FMT_COMPACT) {
    printf(
        ...
        ttc_pct_color(ctx, mem, 80, 95), ttc_color(ctx, COL_RESET), mem, load, rx, tx);
    return;
  }
  ...
  printf(" %sCPU%s   %s%5.1f%%%s  %c\n", ttc_color(ctx, COL_BOLD), ttc_color(ctx, COL_RESET),
         ttc_pct_color(ctx, cpu, 70, 90), cpu, ttc_color(ctx, COL_RESET), sparkchar(cpu));
  printf(" %sMEM%s   %s%5.1f%%%s  %c\n", ttc_color(ctx, COL_BOLD), ttc_color(ctx, COL_RESET),
         ttc_pct_color(ctx, mem, 80, 95), mem, ttc_color(ctx, COL_RESET), sparkchar(mem));
  printf(" %sDISK%s  %s%5.1f%%%s  %s / %s\n", ttc_color(ctx, COL_BOLD), ttc_color(ctx, COL_RESET),
         ttc_pct_color(ctx, disk, 80, 95), disk, ttc_color(ctx, COL_RESET), free_, total);
  printf(" %sNET%s   ↓%s/s  ↑%s/s\n", ttc_color(ctx, COL_BOLD), ttc_color(ctx, COL_RESET), rx, tx);
  printf(" %sLOAD%s  %.2f  %.2f  %.2f\n", ttc_color(ctx, COL_BOLD), ttc_color(ctx, COL_RESET), load,
         m->load_5min / 100.0, m->load_15min / 100.0);
  printf(" %sPROC%s  %u/%u\n", ttc_color(ctx, COL_BOLD), ttc_color(ctx, COL_RESET), m->nr_running,
         m->nr_total);
}
...
void ttc_print_ring_level(const struct ttc_ctx* ctx, int level, const struct ttr_meta* meta,
  ...
  double fill_pct = meta->capacity > 0 ? (double)filled / meta->capacity * 100.0 : 0.0;

  /* ASCII fill bar (20 chars wide) */
  char bar[21] = {0};
  int filled_chars = (int)(fill_pct / 100.0 * 20);
  for (int i = 0; i < 20; i++) bar[i] = i < filled_chars ? '#' : '.';

  printf(" %sL%d%s %-6s  [%s%s%s] %3.0f%%  %u/%u  %s→%s\n", ttc_color(ctx, COL_BOLD), level,
         ttc_color(ctx, COL_RESET), label, ttc_pct_color(ctx, fill_pct, 70, 90), bar,
         ttc_color(ctx, COL_RESET), fill_pct, filled, meta->capacity, first, last);
}
