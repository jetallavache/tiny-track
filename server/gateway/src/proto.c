...
uint8_t ttg_proto_checksum(const struct tt_proto_header* hdr) {
  ...
  for (size_t i = 0; i < sizeof(*hdr); i++) {
    if (i != offsetof(struct tt_proto_header, checksum)) cs ^= b[i];
  }
  return cs;
}

int ttg_proto_validate(const struct tt_proto_header* hdr) {
  if (hdr->magic != TT_PROTO_MAGIC) return -1;
  if (hdr->version != TT_PROTO_V1 && hdr->version != TT_PROTO_V2) return -1;
  if (ttg_proto_checksum(hdr) != hdr->checksum) return -1;
  return 0;
}
...
size_t ttg_proto_build(void* buf, size_t buf_size, uint8_t version, uint8_t type,
  ...
  if (payload_len > 0) memcpy((uint8_t*)buf + sizeof(*hdr), payload, payload_len);
