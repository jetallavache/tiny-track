...
static int b64_encode_single(int c) {
  if (c < 26) {
    return c + 'A';
  } else if (c < 52) {
    return c - 26 + 'a';
...
static int b64_decode_single(int c) {
  if (c >= 'A' && c <= 'Z') {
    return c - 'A';
  } else if (c >= 'a' && c <= 'z') {
    return c + 26 - 'a';
...
size_t ttg_b64_update(unsigned char ch, char* to, size_t n) {
  ...
  if (rem == 0) {
    ...
    to[n] = (char)b64_encode_single(to[n] | (ch >> 4));
    to[++n] = (char)((ch & 15) << 2);
  } else {
    to[n] = (char)b64_encode_single(to[n] | (ch >> 6));
    to[++n] = (char)b64_encode_single(ch & 63);
    n++;
  }
  return n;
}
