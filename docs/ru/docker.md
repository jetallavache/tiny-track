...
| Параметр | Поведение |
|----------|-----------|
| `hostname` | UTS namespace контейнера (не хоста) |
| `os_type` | Хостовое ядро из `/host/proc/sys/kernel/ostype` ✓ |
| `uptime` | Аптайм хоста из `/host/proc/uptime` ✓ |
| CPU/RAM/Net | Данные хоста через `/host/proc` ✓ |
| Disk | `statvfs(/host/rootfs)` — диск хоста ✓ |
