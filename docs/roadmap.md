...
- [?] Пересмотреть поле `alert` в протоколе: нужно ли оно? Если да — определить семантику → решение принять в Блоке 2 (4-й буфер alerts)
...
- [ ] Рассмотреть 4-й буфер: стек критических событий (alerts) с длительным хранением — определяет семантику `PKT_ALERT` и `CMD_SET_ALERTS`
...
- [ ] `HTTP API /api/metrics/live`: привести к Prometheus-совместимому формату (см. Блок 10) или убрать — оставлять дублирование WS нецелесообразно
- [ ] Пул процессов/потоков для изоляции клиентов: зафиксировать решение — текущая неблокирующая epoll-модель остаётся основной; пул рассматривать только при необходимости изоляции по памяти (> N клиентов)
...
- [x] Все страницы приведены к единому оформлению: `LiveExample` (preview + code tabs) + `PropsTable` + `Divider`; добавлены страницы `DiskMap`, `SystemLoad`, `Metrics3D` в новом формате
...
- [x] `mockData` — генератор пресетов фейковых данных: `idle`, `normal`, `highLoad`, `diskFull`, `netSaturated`, `critical`; используется во всех stories
- [x] Stories для `DiskMap`: Default, Ring, Matrix, NoSegments, CustomColors, Small, Large, HighUsage
- [x] Stories для `Metrics3D` v2: Default, AllMetrics, Large, DeepHistory, HighLoad, Paused
- [x] Stories для `SystemLoad`: AllSizes, HighLoad, Idle, Rising, Falling
...
- [ ] `tiny-cli logs`: улучшить диагностику при отсутствии прав на journal — показывать fallback из файла лога или syslog
- [ ] `tiny-cli logs --follow`: режим слежения (аналог `journalctl -f`) — реализовать через `inotify` или polling файла лога
...
**Новые компоненты:**
- [x] `SystemLoad` — лаконичная визуализация общей нагрузки системы: анализ `load_1min/5min/15min` + `nr_running/nr_total`; цветовая градация (ok/warn/crit); крупный шрифт; индикатор тренда ↑↓ (растёт/снижается); расчёт нагрузки исправлен — порог тренда синхронизирован с `alerts.ts` (diff > 20 integer units = 0.20 load)
...
- [x] Высота куба = текущее значение метрики (высокий куб = высокая нагрузка); цветовая градация ok/warn/crit по токенам темы
- [x] Hover-тултип на объекте: проценты, абсолютные значения, скорости трафика (net tx/rx), load average — всё в одном попапе
- [x] Текстовая сводка в углу экрана: все метрики одним взглядом (текущее / max / min за сессию)
- [x] Сессионные min/max: накапливаются с момента подключения, отображаются рядом с текущим значением
- [ ] Клик по объекту — открывает детальную панель с историей последних 60 значений (sparkline/line chart внутри 3D-оверлея)
- [x] Кнопки переключения камеры: общий план / приближение к выбранной метрике / вид сверху (top-down)
- [x] Чекбоксы для скрытия отдельных метрик (связаны с `metrics?: MetricType[]` prop)
- [x] Кнопка паузы — заморозить текущий кадр (анимация и обновление данных останавливаются), чтобы рассмотреть детали
...
**MetricsBar — интерактивные попапы:**
- [x] Каждый badge кликабелен — открывает попап с детальной информацией (`BadgePopup`)
- [x] CPU: usage %, running/total procs; MEM: usage %; DISK: usage %, total, free; LOAD: 1m/5m/15m avg; NET: TX/RX; PROC: running/total
- [x] sysInfo badges: hostname → OS type; uptime → секунды; ringbufInfo → slots L1/L2/L3 + интервалы агрегации
- [x] Попап позиционируется через `position: fixed` + `usePopupPosition` / `usePopupClamp` — не выходит за viewport ни на десктопе ни на мобильном
- [x] Ширина попапа: 70% от ширины MetricsBar на десктопе, `vw - 16px` на мобильном (`< 640px`); ширина отслеживается через `ResizeObserver`
- [x] Закрытие по клику/тапу снаружи; предпочитает открытие вниз, при нехватке места — вверх

**MetricsBar — AlertLamps переработка:**
- [x] `AlertState` переработан: убраны `load` и `spike`, добавлен `sys: 'ok' | 'falling' | 'rising' | 'spike' | 'off'`
- [x] `sys` лампа — иконка ОС (`OsIcon`): детект по `sysinfo.osType` (linux → Tux, darwin → Apple, windows → плитки); стрелка-оверлей `↑`/`↓`/`⚡` в правом нижнем углу
- [x] Цвета `sys`: falling → зелёный, rising → оранжевый, spike → красный, ok → серый
- [x] `netAnalysis()` — анализ сетевой нагрузки: idle / normal / high (>50 MB/s) / saturated (>200 MB/s); используется в `detectAlerts` и `buildAlertState`
- [x] Попап AlertLamps: `load-*` алерты не дублируются — рендерятся только через `sys`-блок; `net-high`/`net-saturated` в отдельных строках
- [x] Иконки cpu/mem/disk/net загораются: warn → жёлтый, crit → красный (через точечный оверрайд токена темы); ok → штатный цвет с opacity 0.5

**MetricsBar — иконки в стиле Tux:**
- [x] Все иконки (`MetricIcon`) переработаны в едином стиле Tux penguin: базовый силуэт `TuxBase` + уникальный атрибут метрики
- [x] CPU — чип на животе; MEM — планка RAM в крыле; DISK — диск у ног; NET — антенна + wifi-дуги на голове; PROC — терминал на животе; LOAD — гистограмма; SPIKE — молния
- [x] `MetricIcon` принимает `t?: TtTheme` — цвет тела пингвина берётся из токенов темы
...
- [x] Новый компонент `DiskMap` — отображает использование диска с разбивкой по категориям
- [x] Два режима отображения (переключаемые): **ring** (круговой бар, как donut chart) и **matrix** (поле квадратиков, например 30×80, каждый квадрат = единица пространства)
- [x] Базовые данные: общий объём и занятое место из `tinytd` (disk usage из метрик)
- [x] Внешние категории через prop `segments?: DiskSegment[]` — данные приходят от стороннего источника (например, RESTful API бэкенда): `{ label: string, bytes: number, color?: string }`; остаток (не покрытый сегментами) отображается как «other»
- [x] Тип `DiskSegment` экспортируется из `tinytsdk`
- [x] Цвета категорий: дефолтная палитра из токенов темы; переопределяется через `color` в сегменте
- [x] Легенда: список категорий с цветом, меткой и процентом/объёмом
- [x] Props: `segments?`, `mode?: "ring" | "matrix"`, `matrixCols?: number` (default 80), `matrixRows?: number` (default 30), `size?`, `theme?`
- [x] Demo-страница и Storybook story с `MockProvider` + пример сегментов (db, uploads, other)
...
## Блок 12 — SDK: оптимизация бандла и безопасность

**Цель:** уменьшить размер бандла, убрать лишнее, проверить уязвимости.

**Анализ и очистка:**
- [ ] Замерить размер текущих бандлов: `dist/index.esm.js`, `dist/react.esm.js`, `dist/react.cjs.js` — зафиксировать baseline (gzip)
- [ ] Проверить tree-shaking: убедиться что `three` не попадает в core-бандл (`index.esm.js`) — только в react-бандл через `Metrics3D`
- [ ] Проверить что `@types/three` не попадает в `dependencies` (только `devDependencies`) — сейчас он в `dependencies`, это лишнее
- [ ] Аудит экспортов `src/index.ts` и `src/react/index.ts`: убрать реэкспорты которые нигде не используются снаружи
- [ ] Проверить `tslib` — используется ли в бандле или только в dev; если не нужен в runtime — убрать из зависимостей

**Оптимизация rollup:**
- [ ] Добавить `@rollup/plugin-terser` для минификации ESM/CJS бандлов
- [ ] Добавить IIFE/UMD output в `rollup.config.js` для CDN (`tinytsdk.min.js`) — связано с Блоком 11
- [ ] Включить `treeshake: { moduleSideEffects: false }` в rollup — агрессивный tree-shaking
- [ ] Добавить `rollup-plugin-visualizer` для визуализации состава бандла (запускать вручную, не в CI)
- [ ] Сравнить размеры до/после минификации, зафиксировать в `docs/dev-history.md`

**Аудит уязвимостей:**
- [ ] Запустить `npm audit` в `sdk/` — зафиксировать найденные CVE
- [ ] Запустить `npm audit` в `demo/` — отдельно, у него свои зависимости
- [ ] Исправить уязвимости с severity `high` / `critical` — обновить пакеты или заменить
- [ ] Настроить `npm audit --audit-level=high` в CI (`sdk-publish.yml`) — блокировать публикацию при критических CVE
- [ ] `[?]` Рассмотреть `socket.dev` или `snyk` для мониторинга зависимостей в реальном времени

---

## Блок 13 — Dashboard v2: информативность и управление сервером

**Цель:** сделать Dashboard самостоятельным инструментом администратора — не дублировать MetricsPanel, а расширить его возможностями управления и визуализации.

**Визуальный стиль и компоновка:**
- [ ] Переработать layout: убрать дублирование с MetricsPanel — Dashboard показывает агрегированную картину, MetricsPanel — детали в реальном времени
- [ ] Добавить спидометры (gauge/dial) для CPU, MEM, DISK — SVG-компоненты, цветовая градация ok/warn/crit по токенам темы
- [ ] Перенести `TimeSeriesChart` в отдельную секцию Dashboard (не поверх метрик) — горизонтальный ряд графиков под спидометрами
- [ ] Добавить секцию «System» с hostname, OS, uptime, kernel version из `sysinfo` — крупно, в шапке Dashboard
- [ ] Добавить секцию «Ring Buffers» — визуализация заполненности L1/L2/L3 (прогресс-бары или мини-Timeline)
- [ ] Compact/expanded mode: в compact — только спидометры + статус; в expanded — полная раскладка

**Управление сервером (tinytd):**
- [ ] Кнопки Start / Stop стриминга — уже есть `CMD_START`/`CMD_STOP`, вынести в отдельный блок управления
- [ ] Изменение интервала сбора: input + кнопка Apply → `CMD_SET_INTERVAL`; показывать текущий интервал из `sysinfo`
- [ ] Переключатель типа агрегации: avg / max / min — влияет на отображение в графиках Dashboard (локальный state, не команда серверу)
- [ ] `[?]` `CMD_SET_ALERTS` — управление порогами алертов прямо из Dashboard (реализовать после того как семантика определена в Блоке 2)
- [ ] `[?]` Кнопка «Reset session» — сброс накопленных min/max за сессию (локальный state)

**Интерактив:**
- [ ] Клик по спидометру — разворачивает детальный график этой метрики (inline expand, без модалки)
- [ ] Hover на спидометре — тултип с текущим значением, min/max за сессию, единицы измерения
- [ ] Keyboard shortcuts: `s` — start/stop, `r` — reset session, `1/2/3` — переключение уровня буфера в графиках
- [ ] WS-консоль (уже есть) — вынести в collapsible секцию внизу, скрыта по умолчанию

**Demo и Storybook:**
- [ ] Обновить demo-страницу Dashboard: показать новый layout, управляющие элементы, спидометры
- [ ] Добавить Storybook story `Dashboard/ControlPanel` с MockProvider — демонстрирует Start/Stop, изменение интервала
- [ ] Добавить story `Dashboard/Gauges` — спидометры в разных состояниях (ok/warn/crit/idle)

---

## Блок 14 — tinytsdk-lite: минимальный npm-пакет

**Цель:** отдельный пакет `tinytsdk-lite` — только WebSocket-клиент и протокол, без React, без three.js, минимальный размер.

**Состав lite-пакета:**
- [ ] Определить границу: `tinytsdk-lite` включает только `TinyTrackClient`, `proto.ts` (константы + парсеры), типы (`TtMetrics`, `TtSysInfo`, `TtConfig`, `TtAck`, `TtStats`, `TtHistoryResp`) — без React-хуков, без компонентов, без `Metrics3D`
- [ ] Создать `sdk-lite/` как отдельный пакет (собственный `package.json`, `tsconfig.json`, `rollup.config.js`) — не монорепо, просто второй каталог рядом с `sdk/`
- [ ] Бандл: только ESM + CJS, без IIFE (IIFE — в полном пакете); цель — < 5 KB gzip
- [ ] Нет peer-зависимостей: `react` и `three` не нужны; нет `peerDependencies` вообще
- [ ] Версионирование: синхронизировать мажорную версию с `tinytsdk` (например, `tinytsdk@0.5.0` → `tinytsdk-lite@0.5.0`); patch/minor могут расходиться

**Разделение кода:**
- [ ] Вынести `client.ts` и `proto.ts` в `sdk/src/core/` — общий источник для обоих пакетов (symlink или копирование при сборке)
- [ ] `[?]` Рассмотреть монорепо (npm workspaces / turborepo) если дублирование кода станет проблемой — пока отложить, решить через symlink

**CI/CD:**
- [ ] Добавить workflow `sdk-lite-publish.yml`: публикация `tinytsdk-lite` при теге `sdk-lite/v*`
- [ ] `npm audit` для `sdk-lite/` в CI — отдельный шаг
- [ ] Добавить badge `tinytsdk-lite` в `README.md` рядом с `tinytsdk`

**Документация:**
- [ ] Добавить в demo-сайт страницу «tinytsdk-lite»: когда использовать lite vs full, пример подключения, размер бандла
- [ ] Пример в docs: vanilla JS без React — подключение через `tinytsdk-lite`, вывод метрик в DOM без фреймворка

---

## Отложено / под вопросом

- `[x]` Storybook vs demo: решено — оба существуют параллельно; Storybook — изолированные stories с MockProvider, demo — реальное подключение к серверу
