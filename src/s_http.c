...
struct str_t *s_http_get_header(struct s_http_message *h, const char *name) {
...
static bool s_http_parse_headers(const char *s, const char *end,
                                 struct s_http_header *h, size_t max_hdrs) {
  size_t i, n;
...
int s_http_parse(const char *s, size_t len, struct s_http_message *hm) {
  int is_response, req_len = s_http_get_request_len((unsigned char *)s, len);
  ...
  if ((cl = s_http_get_header(hm, "Content-Length")) != NULL) {
...
void s_http_reply(struct s_conn *c, int code, const char *headers,
                  const char *fmt, ...) {
  va_list ap;
  size_t len;
...
// void s_http_serve_file(struct s_connection *c, struct s_http_message *hm,
//                         const char *path,
//                         const struct s_http_serve_opts *opts) {
...
//     struct str_t *ae = s_http_get_header(hm, "Accept-Encoding");
//     if (ae != NULL) {
...
//     s_http_reply(c, 404, opts->extra_headers, "Not found\n");
//     s_fs_close(fd);
//     // NOTE: s_http_etag() call should go first!
//   } else if (s_http_etag(etag, sizeof(etag), size, mtime) != NULL &&
//              (inm = s_http_get_header(hm, "If-None-Match")) != NULL &&
//              str_tcasecmp(*inm, str_t(etag)) == 0) {
//     s_fs_close(fd);
//     s_http_reply(c, 304, opts->extra_headers, "");
//   } else {
//     int n, status = 200;
//     char range[100];
//     size_t r1 = 0, r2 = 0, cl = size;
//     // Handle Range header
//     struct str_t *rh = s_http_get_header(hm, "Range");
//     range[0] = '\0';
...
// static int uri_to_path2(struct s_conn *c, struct s_http_message *hm,
...
//   if (n + 2 >= path_size) {
//     s_http_reply(c, 400, "", "Exceeded path size");
//     return -1;
//   }
//   path[path_size - 1] = '\0';
...
//     s_http_reply(c, 400, "", "Invalid path");
//     return -1;
//   }
//   n = strlen(path);
...
// static int uri_to_path(struct s_conn *c, struct s_http_message *hm,
//                        const struct s_http_serve_opts *opts, char *path,
//                        size_t path_size) {
//   struct s_fs *fs = opts->fs == NULL ? &s_fs_posix : opts->fs;
...
// void s_http_serve_dir(struct s_conn *c, struct s_http_message *hm,
//                        const struct s_http_serve_opts *opts) {
...
// void s_http_creds(struct s_http_message *hm, char *user, size_t userlen,
//                    char *pass, size_t passlen) {
//   struct str_t *v = s_http_get_header(hm, "Authorization");
//   user[0] = pass[0] = '\0';
...
//   } else if ((v = s_http_get_header(hm, "Cookie")) != NULL) {
...
// long s_http_upload(struct s_connection *c, struct s_http_message *hm,
...
//   if (hm->body.len == 0) {
//     s_http_reply(c, 200, "", "%ld", res);  // Nothing to write
//   } else if (file[0] == '\0') {
//     s_http_reply(c, 400, "", "file required");
//     res = -1;
//   } else if (s_path_is_sane(str_t(file)) == false) {
//     s_http_reply(c, 400, "", "%s: invalid file", file);
//     res = -2;
//   } else if (offset < 0) {
//     s_http_reply(c, 400, "", "offset required");
//     res = -3;
//   } else if ((size_t)offset + hm->body.len > max_size) {
//     s_http_reply(c, 400, "", "%s: over max size of %lu", path,
...
//       s_http_reply(c, 400, "", "%s: offset mismatch", path);
//       res = -5;
//     } else if ((fd = s_fs_open(fs, path, MG_FS_WRITE)) == NULL) {
//       s_http_reply(c, 400, "", "open(%s)", path);
//       res = -6;
//     } else {
//       res = offset + (long)fs->wr(fd->fd, hm->body.buf, hm->body.len);
//       s_fs_close(fd);
//       s_http_reply(c, 200, "", "%ld", res);
//     }
//   }
//   return res;
// }

int s_http_status(const struct s_http_message *hm) { return atoi(hm->uri.buf); }
...
static void http_cb(struct s_conn *c, int ev, void *ev_data) {
  if (ev == S_EVENT_READ || ev == S_EVENT_CLOSE ||
    ...
    while (c->is_resp == 0 && ofs < c->recv.len) {
      ...
      int n = s_http_parse(buf, c->recv.len - ofs, &hm);
      ...
      if ((te = s_http_get_header(&hm, "Transfer-Encoding")) != NULL) {
      ...
      } else if (s_http_get_header(&hm, "Content-length") == NULL) {
      ...
      if (c->is_accepted && !c->is_resp) {
        struct str_t *cc = s_http_get_header(&hm, "Connection");
...
struct s_conn *s_http_connect(struct s_mgr *mgr, const char *url,
                              s_event_handler_t fn, void *fn_data) {
  return s_net_connect_svc(mgr, url, fn, fn_data, http_cb, NULL);
}

struct s_conn *s_http_listen(struct s_mgr *mgr, const char *url,
                             s_event_handler_t fn, void *fn_data) {
