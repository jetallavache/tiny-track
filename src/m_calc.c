...
bool readpr_stat(struct calc_stat *ps) {
  bool success = true;
  void *buffer;
  int n;
...
bool readpr_meminf(struct calc_meminfo *pm) {
  bool success = true;
  void *buffer;
  int n;
...
bool readpr_net(struct calc_net *pn) {
  void *buffer;
  bool success = true;
  int n;
...
bool readpr_lavg(struct calc_loadavg *pl) {
  void *buffer;
  bool success = true;
  int n;
...
struct calc_du readfs_du(const char *path) {
  struct calc_du du = {0};
  struct statvfs fs;
...
struct calc_du du_cached(struct m_state *st) {
...
float m_calc_cpu_usage(struct m_state *st) {
  unsigned long ctotal, ptotal, cwork, pwork, difftotal, diffwork;
  float usage = 0.0;
  struct calc_stat curr = {0};
  if (!readpr_stat(&curr)) {
...
void m_calc_net(struct m_state *st, unsigned long *rx, unsigned long *tx) {
  struct calc_net curr = {0};
  if (!readpr_net(&curr)) {
...
struct calc_du m_calc_disk_usage(struct m_state *st) {
  struct calc_du du = du_cached(st);
  return du;
}