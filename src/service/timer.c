...
void timer_init(struct timer** head, struct timer* t, uint64_t ms,
...
void timer_free(struct timer** head, struct timer* t) {
  while (*head && *head != t) head = &(*head)->next;
  if (*head) *head = t->next;
}

// t: expiration time, prd: period, now: current time. Return true if expired
bool timer_expired(uint64_t* t, uint64_t prd, uint64_t now) {
...
void timer_poll(struct timer** head, uint64_t now_ms) {
  struct timer *t, *tmp;
  for (t = *head; t != NULL; t = tmp) {
    bool once = t->expire == 0 && (t->flags & S_TIMER_RUN_NOW) &&
                !(t->flags & S_TIMER_CALLED);  // Handle S_TIMER_NOW only once
    bool expired = timer_expired(&t->expire, t->period_ms, now_ms);
    tmp = t->next;
