...
struct str_t str_s(const char *s) {
  struct str_t str = {(char *)s, s == NULL ? 0 : strlen(s)};
  return str;
}

struct str_t str_n(const char *s, size_t n) {
  struct str_t str = {(char *)s, n};
  return str;
}

static int tolc(char c) { return (c >= 'A' && c <= 'Z') ? c + 'a' - 'A' : c; }
...
bool str_match(struct str_t s, struct str_t p, struct str_t *caps) {
...
// bool span(struct str_t s, struct str_t *a, struct str_t *b, char sep) {
//   if (s.len == 0 || s.buf == NULL) {
...
bool str_to_num(struct str_t str, int base, void *val, size_t val_len) {
