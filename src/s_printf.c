...
static size_t s_dtoa(char *dst, size_t dstlen, double d, int width, bool tz) {
  ...
  int i, s = 0, n = 0, e = 0;
  double t, mul, saved;
  if (d == 0.0) return snprintf(dst, dstlen, "%s", "0");
  if (xisinf(d)) return snprintf(dst, dstlen, "%s", d > 0 ? "inf" : "-inf");
  if (xisnan(d)) return snprintf(dst, dstlen, "%s", "nan");
  if (d < 0.0) d = -d, buf[s++] = '-';
...
size_t s_vxprintf(void (*out)(char, void *), void *param, const char *fmt,
                  va_list *ap) {
  size_t i = 0, n = 0;
  while (fmt[i] != '\0') {
    if (fmt[i] == '%') {
...
/* Существует лиреализация xprintf в стандартных библиотеках? */

size_t s_xprintf(void (*out)(char, void *), void *ptr, const char *fmt, ...) {
  size_t len = 0;
  va_list ap;
  va_start(ap, fmt);
  len = s_vxprintf(out, ptr, fmt, &ap);
  va_end(ap);
  return len;
}
...
static void s_pfn_iobuf_private(char ch, void *param, bool expand) {
  struct s_iobuf *io = (struct s_iobuf *)param;
  if (expand && io->len + 2 > io->size) s_iobuf_resize(io, io->len + 2);
  if (io->len + 2 <= io->size) {
...
void s_pfn_iobuf(char ch, void *param) { s_pfn_iobuf_private(ch, param, true); }

size_t s_vsnprintf(char *buf, size_t len, const char *fmt, va_list *ap) {
  struct s_iobuf io = {(uint8_t *)buf, len, 0, 0};
  size_t n = s_vxprintf(s_putchar_iobuf_static, &io, fmt, ap);
  if (n < len) buf[n] = '\0';
  return n;
}
...
char *s_vmprintf(const char *fmt, va_list *ap) {
  struct s_iobuf io = {0, 0, 0, 256};
  s_vxprintf(s_pfn_iobuf, &io, fmt, ap);
  return (char *)io.buf;
}
...
void s_pfn_stdout(char c, void *param) {
  putchar(c);
  (void)param;
}

static size_t print_ip4(void (*out)(char, void *), void *arg, uint8_t *p) {
  return s_xprintf(out, arg, "%d.%d.%d.%d", p[0], p[1], p[2], p[3]);
}

// static size_t print_ip6(void (*out)(char, void *), void *arg, uint16_t *p) {
//   return s_xprintf(out, arg, "[%x:%x:%x:%x:%x:%x:%x:%x]", s_ntohs(p[0]),
...
size_t s_print_ip(void (*out)(char, void *), void *arg, va_list *ap) {
  struct s_addr *addr = va_arg(*ap, struct s_addr *);
  //   if (addr->is_ip6) return print_ip6(out, arg, (uint16_t *) addr->ip);
  return print_ip4(out, arg, (uint8_t *)&addr->ip);
}

size_t s_print_ip_port(void (*out)(char, void *), void *arg, va_list *ap) {
  struct s_addr *a = va_arg(*ap, struct s_addr *);
  return s_xprintf(out, arg, "%M:%hu", s_print_ip, a, ntohs(a->port));
}

size_t s_print_mac(void (*out)(char, void *), void *arg, va_list *ap) {
  uint8_t *p = va_arg(*ap, uint8_t *);
  return s_xprintf(out, arg, "%02x:%02x:%02x:%02x:%02x:%02x", p[0], p[1], p[2],
                   p[3], p[4], p[5]);
}
...
static char escape(int c) { return esc(c, true); }
