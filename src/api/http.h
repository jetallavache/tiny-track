...
int api_http_parse(const char* s, size_t len, struct api_http_message*);
...
// void s_http_delete_chunk(struct s_conn *c, struct api_http_message *hm);

struct s_conn* api_http_listen(struct s_mgr*, const char* url,
                               s_event_handler_t fn, void* fn_data);
struct s_conn* api_http_connect(struct s_mgr*, const char* url,
                                s_event_handler_t fn, void* fn_data);

// void s_http_serve_dir(struct s_conn *, struct api_http_message *hm,
//                        const struct api_http_serve_opts *);
// void s_http_serve_file(struct s_conn *, struct api_http_message *hm,
//                         str_t path, const struct api_http_serve_opts *);

void api_http_reply(struct s_conn*, int status_code, const char* headers,
                    const char* body_fmt, ...);
struct str_t* api_http_get_header(struct api_http_message*, const char* name);
...
// void s_http_creds(struct api_http_message *, char *, size_t, char *, size_t);
// long s_http_upload(struct s_conn *c, struct api_http_message *hm,
