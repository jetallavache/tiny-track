...
size_t api_ws_vprintf(struct s_conn* c, int op, const char* fmt, va_list* ap) {
  size_t len = c->send.len;
  size_t n = s_vxprintf(s_pfn_iobuf, &c->send, fmt, ap);
  api_ws_wrap(c, c->send.len - len, op);
  return n;
}
...
size_t api_ws_printf(struct s_conn* c, int op, const char* fmt, ...) {
  ...
  va_list ap;
  va_start(ap, fmt);
  len = api_ws_vprintf(c, op, fmt, &ap);
  va_end(ap);
  return len;
}
...
size_t api_ws_send(struct s_conn* c, const void* buf, size_t len, int op) {
  uint8_t header[14];
...
static bool s_ws_client_handshake(struct s_conn* c) {
  int n = api_http_get_request_len(c->recv.buf, c->recv.len);
  if (n < 0) {
    ...
    if (n < 15 || memcmp(c->recv.buf + 9, "101", 3) != 0) {
      ...
      if (api_http_parse((char*)c->recv.buf, c->recv.len, &hm)) {
        c->is_websocket = 1;
...
static void s_ws_cb(struct s_conn* c, int ev, void* ev_data) {
  ...
  if (ev == S_EVENT_READ) {
    ...
    while (ws_process(c->recv.buf + ofs, c->recv.len - ofs, &msg) > 0) {
      ...
      struct api_ws_message m = {{s, msg.data_len}, msg.flags};
      ...
      switch (op) {
        ...
        case WS_OP_PING:
          L_DEBUG(("%s", "WS PONG"));
          api_ws_send(c, s, msg.data_len, WS_OP_PONG);
          s_event_call(c, S_EVENT_WS_CTL, &m);
          break;
        case WS_OP_PONG:
          s_event_call(c, S_EVENT_WS_CTL, &m);
        ...
        case WS_OP_CLOSE:
          ...
          api_ws_send(c, m.data.buf, m.data.len, WS_OP_CLOSE);
          c->is_draining = 1;
          break;
        default:
...
struct s_conn* api_ws_connect(struct s_mgr* mgr, const char* url,
...
void api_ws_upgrade(struct s_conn* c, struct api_http_message* hm,
                    const char* fmt, ...) {
  struct str_t* wskey = api_http_get_header(hm, "Sec-WebSocket-Key");
  c->pfn = s_ws_cb;
  c->pfn_data = NULL;
  if (wskey == NULL) {
    api_http_reply(c, 426, "", "WS upgrade expected\n");
    c->is_draining = 1;
  } else {
    struct str_t* wsproto = api_http_get_header(hm, "Sec-WebSocket-Protocol");
    va_list ap;
    va_start(ap, fmt);
...
size_t api_ws_wrap(struct s_conn* c, size_t len, int op) {
  uint8_t header[14], *p;
  size_t header_len = mkhdr(len, op, c->is_client, header);
