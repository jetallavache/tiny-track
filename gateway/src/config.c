...
void ttg_config_load(struct ttg_config *cfg, const char *config_path,
  ...
  tt_config_read_str(config_path, "gateway.user",
                     cfg->user, sizeof(cfg->user), "tinytrack");
  tt_config_read_str(config_path, "gateway.group",
                     cfg->group, sizeof(cfg->group), "tinytrack");
  ...
  tt_config_read_str(config_path, "gateway.listen",
                     cfg->listen, sizeof(cfg->listen), DEFAULT_LISTEN);
  tt_config_read_str(config_path, "storage.live_path",
                     cfg->shm_path, sizeof(cfg->shm_path), DEFAULT_SHM_PATH);
  cfg->update_interval_ms = tt_config_read_int(config_path,
                     "gateway.update_interval", DEFAULT_INTERVAL);
