...
// bool to_size_t(struct tt_util_string str, size_t *val);
// bool to_size_t(struct tt_util_string str, size_t *val) {
...
// //
// // --xyz--
// size_t ttg_http_next_multipart(struct tt_util_string body, size_t ofs,
//                               struct ttg_http_part *part) {
//   struct tt_util_string cd = str_t_n("Content-Disposition", 19);
...
//       struct tt_util_string v = str_t_n(&s[h1 + cd.len + 2], h2 - (h1 +
//       cd.len + 2)); part->name = ttg_http_get_header_var(v, str_t_n("name",
//       4)); part->filename = ttg_http_get_header_var(v, str_t_n("filename",
//       8));
//     }
//     h1 = h2 = h2 + 2;
//   }
...
// void ttg_http_bauth(struct ttg_connection *c, const char *user,
//                    const char *pass) {
//   struct tt_util_string u = str_t(user), p = str_t(pass);
//   size_t need = c->send.len + 36 + (u.len + p.len) * 2;
//   if (c->send.size < need) ttg_iobuf_resize(&c->send, need);
//   if (c->send.size >= need) {
//     size_t i, n = 0;
//     char *buf = (char *)&c->send.buf[c->send.len];
//     memcpy(buf, "Authorization: Basic ", 21);  // DON'T use ttg_send!
//     for (i = 0; i < u.len; i++) {
//       n = ttg_base64_update(((unsigned char *)u.buf)[i], buf + 21, n);
//     }
//     if (p.len > 0) {
//       n = ttg_base64_update(':', buf + 21, n);
//       for (i = 0; i < p.len; i++) {
//         n = ttg_base64_update(((unsigned char *)p.buf)[i], buf + 21, n);
//       }
//     }
//     n = ttg_base64_final(buf + 21, n);
...
// struct tt_util_string ttg_http_var(struct tt_util_string buf, struct
// tt_util_string name) {
//   struct tt_util_string entry, k, v, result = str_t_n(NULL, 0);
...
// int ttg_http_get_var(const struct tt_util_string *buf, const char *name, char
// *dst,
//                     size_t dst_len) {
//   int len;
...
//     len = -1;  // Bad source
//   } else {
//     struct tt_util_string v = ttg_http_var(*buf, str_t(name));
//     if (v.buf == NULL) {
//       len = -4;  // Name does not exist
//     } else {
//       len = ttg_url_decode(v.buf, v.len, dst, dst_len, 1);
...
// int ttg_url_decode(const char *src, size_t src_len, char *dst, size_t
// dst_len,
...
struct tt_util_string* ttg_http_get_header(struct ttg_http_message* h,
                                           const char* name) {
  size_t i, n = strlen(name), max = sizeof(h->headers) / sizeof(h->headers[0]);
  for (i = 0; i < max && h->headers[i].name.len > 0; i++) {
    struct tt_util_string *k = &h->headers[i].name, *v = &h->headers[i].value;
...
static bool ttg_http_parse_headers(const char* s, const char* end,
  ...
  for (i = 0; i < max_hdrs; i++) {
    struct tt_util_string k = {NULL, 0}, v = {NULL, 0};
    if (s >= end)
      return false;
    if (s[0] == '\n' || (s[0] == '\r' && s[1] == '\n'))
      break;
...
int ttg_http_parse(const char* s, size_t len, struct ttg_http_message* hm) {
  ...
  const struct tt_util_string* cl;
  size_t n;
...
// static void ttg_http_vprintf_chunk(struct ttg_connection *c, const char *fmt,
//                                   va_list *ap) {
...
// void ttg_http_printf_chunk(struct ttg_connection *c, const char *fmt, ...) {
//   va_list ap;
//   va_start(ap, fmt);
//   ttg_http_vprintf_chunk(c, fmt, &ap);
//   va_end(ap);
// }

// void ttg_http_write_chunk(struct ttg_connection *c, const char *buf, size_t
// len)
// {
//   ttg_net_printf(c, "%lx\r\n", (unsigned long)len);
//   ttg_send(c, buf, len);
...
// static void http_cb(struct ttg_connection *, int, void *);
// static void restore_http_cb(struct ttg_connection *c) {
//   ttg_fs_close((struct ttg_fd *)c->pfn_data);
//   c->pfn_data = NULL;
//   c->pfn = http_cb;
//   c->is_resp = 0;
// }
...
// static void static_cb(struct ttg_connection *c, int ev, void *ev_data) {
//   if (ev == TTG_EVENT_WRITE || ev == TTG_EVENT_POLL) {
//     struct ttg_fd *fd = (struct ttg_fd *)c->pfn_data;
//     // Read to send IO buffer directly, avoid extra on-stack buffer
//     size_t n, max = MG_IO_SIZE, space;
//     size_t *cl = (size_t *)&c->data[(sizeof(c->data) - sizeof(size_t)) /
//                                     sizeof(size_t) * sizeof(size_t)];
//     if (c->send.size < max) ttg_iobuf_resize(&c->send, max);
...
// static struct tt_util_string ttg_known_types[] = {
...
// static struct tt_util_string guess_content_type(struct tt_util_string path,
// const char *extra)
// {
//   struct tt_util_string entry, k, v, s = str_t(extra), asterisk =
//   str_t_n("*", 1); size_t i = 0;
...
//   for (i = 0; ttg_known_types[i].buf != NULL; i += 2) {
//     if (str_tcmp(path, ttg_known_types[i]) == 0) return ttg_known_types[i +
//     1];
//   }
//   return str_t("text/plain; charset=utf-8");
// }

// static int getrange(struct tt_util_string *s, size_t *a, size_t *b) {
//   size_t i, numparsed = 0;
//   for (i = 0; i + 6 < s->len; i++) {
//     struct tt_util_string k, v = str_t_n(s->buf + i + 6, s->len - i - 6);
...
// void ttg_http_serve_file(struct ttg_connection *c, struct ttg_http_message
// *hm,
//                         const char *path,
//                         const struct ttg_http_serve_opts *opts) {
//   char etag[64], tmp[MG_PATH_MAX];
//   struct ttg_fs *fs = opts->fs == NULL ? &s_fs_posix : opts->fs;
//   struct ttg_fd *fd = NULL;
//   size_t size = 0;
//   time_t mtime = 0;
//   struct tt_util_string *inm = NULL;
//   struct tt_util_string mime = guess_content_type(str_t(path),
//   opts->mime_types); bool gzip = false; if (path != NULL) {
//     // If a browser sends us "Accept-Encoding: gzip", try to open .gz first
//     struct tt_util_string *ae = ttg_http_get_header(hm, "Accept-Encoding");
//     if (ae != NULL) {
//       if (s_match(*ae, str_t("*gzip*"), NULL)) {
//         snprintf(tmp, sizeof(tmp), "%s.gz", path);
//         fd = ttg_fs_open(fs, tmp, FS_READ);
//         if (fd != NULL) gzip = true, path = tmp;
//       }
//     }
//     // No luck opening .gz? Open what we've told to open
//     if (fd == NULL) fd = ttg_fs_open(fs, path, FS_READ);
//   }
//   // Failed to open, and page404 is configured? Open it, then
//   if (fd == NULL && opts->page404 != NULL) {
//     fd = ttg_fs_open(fs, opts->page404, FS_READ);
//     path = opts->page404;
//     mime = guess_content_type(str_t(path), opts->mime_types);
//   }
//   if (fd == NULL || fs->st(path, &size, &mtime) == 0) {
//     ttg_http_reply(c, 404, opts->extra_headers, "Not found\n");
//     ttg_fs_close(fd);
//     // NOTE: ttg_http_etag() call should go first!
//   } else if (s_http_etag(etag, sizeof(etag), size, mtime) != NULL &&
//              (inm = ttg_http_get_header(hm, "If-None-Match")) != NULL &&
//              str_tcasecmp(*inm, str_t(etag)) == 0) {
//     ttg_fs_close(fd);
//     ttg_http_reply(c, 304, opts->extra_headers, "");
//   } else {
//     int n, status = 200;
//     char range[100];
//     size_t r1 = 0, r2 = 0, cl = size;
//     // Handle Range header
//     struct tt_util_string *rh = ttg_http_get_header(hm, "Range");
//     range[0] = '\0';
...
//               "Content-Length: %llu\r\n"
//               "%s%s%s\r\n",
//               status, ttg_http_status_code_str(status), (int)mime.len,
...
// static int uri_to_path2(struct ttg_conn *c, struct ttg_http_message *hm,
//                         struct ttg_fs *fs, struct tt_util_string url, struct
//                         tt_util_string dir, char *path, size_t path_size) {
//   int flags, tmp;
//   // Append URI to the root_dir, and sanitize it
//   size_t n = snprintf(path, path_size, "%.*s", (int)dir.len, dir.buf);
//   if (n + 2 >= path_size) {
//     ttg_http_reply(c, 400, "", "Exceeded path size");
//     return -1;
//   }
//   path[path_size - 1] = '\0';
...
//     ttg_url_decode(hm->uri.buf + url.len, hm->uri.len - url.len, path + n,
//                   path_size - n, 0);
//   }
//   path[path_size - 1] = '\0';  // Double-check
//   if (!s_path_is_sane(str_t_n(path, path_size))) {
//     ttg_http_reply(c, 400, "", "Invalid path");
//     return -1;
//   }
//   n = strlen(path);
//   while (n > 1 && path[n - 1] == '/') path[--n] = 0;  // Trim trailing
//   slashes flags = str_tcmp(hm->uri, str_t("/")) == 0 ? FS_DIR
...
//   } else if ((flags & FS_DIR) && hm->uri.len > 0 &&
...
// static int uri_to_path(struct ttg_conn *c, struct ttg_http_message *hm,
//                        const struct ttg_http_serve_opts *opts, char *path,
//                        size_t path_size) {
//   struct ttg_fs *fs = opts->fs == NULL ? &s_fs_posix : opts->fs;
//   struct tt_util_string k, v, part, s = str_t(opts->root_dir), u = {NULL, 0},
//   p = u; while (s_span(s, &part, &s, ',')) {
...
// void ttg_http_serve_dir(struct ttg_conn *c, struct ttg_http_message *hm,
//                        const struct ttg_http_serve_opts *opts) {
//   char path[TTG_PATH_MAX];
// //   const char *sp = opts->ssi_pattern;
//   int flags = uri_to_path(c, hm, opts, path, sizeof(path));
//   if (flags < 0) {
//     // Do nothing: the response has already been sent by uri_to_path()
//   } else if (flags & FS_DIR) {
//     ttg_http_reply(c, 403, "", "Forbidden\n");
// //   } else if (flags && sp != NULL && ttg_match(str_t(path), str_t(sp),
// NULL))
// {
// //     ttg_http_serve_ssi(c, opts->root_dir, path);
//   } else {
//     ttg_http_serve_file(c, hm, path, opts);
//   }
// }

// static bool ttg_is_url_safe(int c) {
//   return (c >= '0' && c <= '9') || (c >= 'a' && c <= 'z') ||
//          (c >= 'A' && c <= 'Z') || c == '.' || c == '_' || c == '-' || c ==
//          '~';
// }

// size_t ttg_url_encode(const char *s, size_t sl, char *buf, size_t len) {
//   size_t i, n = 0;
//   for (i = 0; i < sl; i++) {
...
//       buf[n++] = s[i];
//     } else {
//       snprintf(&buf[n], 4, "%%%M", ttg_print_hex, 1, &s[i]);
//       n += 3;
//     }
//   }
...
// void ttg_http_creds(struct ttg_http_message *hm, char *user, size_t userlen,
//                    char *pass, size_t passlen) {
//   struct tt_util_string *v = ttg_http_get_header(hm, "Authorization");
//   user[0] = pass[0] = '\0';
//   if (v != NULL && v->len > 6 && memcmp(v->buf, "Basic ", 6) == 0) {
//     char buf[256];
//     size_t n = ttg_base64_decode(v->buf + 6, v->len - 6, buf, sizeof(buf));
...
//   } else if ((v = ttg_http_get_header(hm, "Cookie")) != NULL) {
//     struct tt_util_string t = ttg_http_get_header_var(*v,
//     str_t_n("access_token", 12)); if (t.len > 0) snprintf(pass, passlen,
//     "%.*s", (int)t.len, t.buf);
//   } else {
//     ttg_http_get_var(&hm->query, "access_token", pass, passlen);
//   }
// }

// static struct tt_util_string stripquotes(struct tt_util_string s) {
//   return s.len > 1 && s.buf[0] == '"' && s.buf[s.len - 1] == '"'
//              ? str_t_n(s.buf + 1, s.len - 2)
//              : s;
// }

// struct tt_util_string ttg_http_get_header_var(struct tt_util_string s, struct
// tt_util_string v) {
//   size_t i;
...
// long ttg_http_upload(struct ttg_connection *c, struct ttg_http_message *hm,
//                     struct ttg_fs *fs, const char *dir, size_t max_size) {
//   char buf[20] = "0", file[MG_PATH_MAX], path[MG_PATH_MAX];
//   long res = 0, offset;
//   ttg_http_get_var(&hm->query, "offset", buf, sizeof(buf));
//   ttg_http_get_var(&hm->query, "file", file, sizeof(file));
//   offset = strtol(buf, NULL, 0);
//   snprintf(path, sizeof(path), "%s%c%s", dir, MG_DIRSEP, file);
//   if (hm->body.len == 0) {
//     ttg_http_reply(c, 200, "", "%ld", res);  // Nothing to write
//   } else if (file[0] == '\0') {
//     ttg_http_reply(c, 400, "", "file required");
//     res = -1;
//   } else if (s_path_is_sane(str_t(file)) == false) {
//     ttg_http_reply(c, 400, "", "%s: invalid file", file);
//     res = -2;
//   } else if (offset < 0) {
//     ttg_http_reply(c, 400, "", "offset required");
//     res = -3;
//   } else if ((size_t)offset + hm->body.len > max_size) {
//     ttg_http_reply(c, 400, "", "%s: over max size of %lu", path,
...
//       ttg_http_reply(c, 400, "", "%s: offset mismatch", path);
//       res = -5;
//     } else if ((fd = ttg_fs_open(fs, path, FS_WRITE)) == NULL) {
//       ttg_http_reply(c, 400, "", "open(%s)", path);
//       res = -6;
//     } else {
//       res = offset + (long)fs->wr(fd->fd, hm->body.buf, hm->body.len);
//       ttg_fs_close(fd);
//       ttg_http_reply(c, 200, "", "%ld", res);
//     }
//   }
//   return res;
// }
...
static void http_cb(struct ttg_conn* c, int ev, void* ev_data) {
  if (ev == TTG_EVENT_READ || ev == TTG_EVENT_CLOSE ||
    ...
    while (c->is_resp == 0 && ofs < c->recv.len) {
      ...
      struct tt_util_string* te;  // Transfer - encoding header
      ...
      if (c->is_accepted && !c->is_resp) {
        struct tt_util_string* cc = ttg_http_get_header(&hm, "Connection");
...
struct ttg_conn* api_http_connect(struct ttg_mgr* mgr, const char* url,
