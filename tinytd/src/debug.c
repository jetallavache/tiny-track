...
void ttd_debug_dump_collector(const struct tt_proto_metrics* s) {
  char ts[16];
...
void ttd_debug_dump_l1(const void* live_addr, uint32_t l1_capacity) {
  ...
  const uint8_t* data =
      (const uint8_t*)live_addr + ttr_layout_l1_offset();
  ...
  fprintf(stderr, "[DBG L1] head=%u  filled=%u/%u  last %u entries:\n",
          head, filled, l1_capacity, show);

  for (uint32_t i = 0; i < show; i++) {
    uint32_t idx = (head - show + i + l1_capacity) % l1_capacity;
    const struct tt_proto_metrics* s =
        (const struct tt_proto_metrics*)(data + idx * cell_size);
    char ts[16];
    fmt_ts(s->timestamp, ts, sizeof(ts));
    fprintf(stderr, "  [%u] ts=%s  cpu=%.2f%%  mem=%.2f%%  rx=%u  tx=%u\n",
            idx, ts, s->cpu_usage / 100.0, s->mem_usage / 100.0,
            s->net_rx, s->net_tx);
  }
}
...
void ttd_debug_dump_agg(int level, const struct tt_proto_metrics* agg,
  ...
  fprintf(stderr,
          "[DBG L%d agg] head=%u/%u  ts=%s"
          "  cpu=%.2f%%  mem=%.2f%%"
          "  rx=%u  tx=%u"
          "  load=%.2f/%.2f/%.2f\n",
          level, head, capacity, ts,
          agg->cpu_usage / 100.0,
          agg->mem_usage / 100.0,
          agg->net_rx,
          agg->net_tx,
